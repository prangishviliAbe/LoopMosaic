/**
 * LoopMosaic - JetSmartFilters Integration V9
 * 
 * COMPREHENSIVE SUPPORT: All JSF filter types
 * - Search input
 * - Checkboxes 
 * - Radio buttons
 * - Select dropdowns
...
(function ($) {
    ...
    const LoopMosaicFilters = {
        ...
        init: function () {
            console.log('LoopMosaic V9: Filters Init Started');
            ...
            // V9: Bind ALL JSF filter types
            this.bindAllJSFFilters();

            console.log('LoopMosaic V9: Filters Init Complete');
        },

        /**
         * V9: Bind to ALL JetSmartFilters filter types
         */
        bindAllJSFFilters: function () {
            const self = this;

            console.log('LoopMosaic V9: Binding all JSF filter types...');

            // Find all filter widgets targeting loop-mosaic
            const $allFilters = $('[data-content-provider="loop-mosaic"], [data-provider-id="loop-mosaic"]');
            console.log('LoopMosaic V9: Found filter widgets targeting loop-mosaic:', $allFilters.length);

            // Bind each filter type
            ...
            // Also look for Apply/Reset buttons
            this.bindApplyButtons();

            // Late binding after DOM settles
            setTimeout(function () {
        ...
        /**
         * Late binding for dynamically loaded filters
         */
        bindAllJSFFilters_Late: function () {
            console.log('LoopMosaic V9: Late filter binding...');
            this.bindSearchFilters();
            this.bindCheckboxFilters();
            this.bindRadioFilters();
            this.bindSelectFilters();
        ...
        bindSearchFilters: function () {
            ...
            $('[class*="jet-smart-filters"] input[type="search"], [class*="jet-smart-filters"] input[type="text"], .jet-search-filter__input').each(function () {
                ...
                console.log('LoopMosaic V9: Binding SEARCH for:', queryId);
        ...
        bindCheckboxFilters: function () {
            ...
            $('.jet-checkboxes-list input[type="checkbox"], .jet-smart-filters-checkboxes input[type="checkbox"]').each(function () {
                ...
                console.log('LoopMosaic V9: Binding CHECKBOX for:', queryId);
        ...
        bindRadioFilters: function () {
            ...
            $('.jet-radio-list input[type="radio"], .jet-smart-filters-radio input[type="radio"]').each(function () {
                ...
                console.log('LoopMosaic V9: Binding RADIO for:', queryId);
        ...
        /**
         * Select/Dropdown Filter Binding
         */
        bindSelectFilters: function () {
            const self = this;

            // JSF Select dropdowns
            $('.jet-select select, .jet-smart-filters-select select, [class*="jet-filter"] select').each(function () {
                ...
                console.log('LoopMosaic V9: Binding SELECT for:', queryId);
        ...
        bindRangeFilters: function () {
            ...
            $('.jet-range__slider, .jet-smart-filters-range, [class*="jet-range"]').each(function () {
                ...
                console.log('LoopMosaic V9: Binding RANGE for:', queryId);

                // Range sliders usually have inputs or use jQuery UI slider
                const $inputs = $range.find('input');
                $inputs.on('change', function () {
                    self.scheduleFilter(queryId, 500);
                });

                // Also try to bind to slider events
        ...
        bindDateFilters: function () {
            ...
            $('.jet-date-period, .jet-smart-filters-date-period, [class*="jet-date"]').each(function () {
                ...
                console.log('LoopMosaic V9: Binding DATE for:', queryId);
        ...
        bindApplyButtons: function () {
            ...
            $('.jet-smart-filters-apply, .apply-filters__button, [class*="jet-apply"]').each(function () {
                ...
                console.log('LoopMosaic V9: Binding APPLY button for:', queryId);
            ...
            $('.jet-smart-filters-remove, .jet-remove-all-filters, [class*="jet-remove"]').each(function () {
                ...
                const queryId = self.getQueryIdFromElement($btn);
                console.log('LoopMosaic V9: Binding RESET button for:', queryId);
        ...
        /**
         * Get Query ID from filter element
         */
        getQueryIdFromElement: function ($el) {
            // Try to find query-id from parent widget
            const $widget = $el.closest('[data-query-id]');
            if ($widget.length) {
                let qid = $widget.data('query-id');
                // Handle duplicated value like "loop-mosaic loop-mosaic"
            ...
            if ($provider.length) {
                return 'loop-mosaic'; // Default if provider is set
            }

            return 'loop-mosaic'; // Fallback
        },

        /**
         * Schedule filter with debounce
         */
        ...
        /**
         * Trigger filter - collect all filter values and send AJAX
         */
        triggerFilter: function (queryId) {
            const self = this;
            const filters = this.collectFilterValues(queryId);

            console.log('LoopMosaic V9: Triggering filter for', queryId, filters);

            this.performFilterAjax(queryId, filters);
        },

        /**
         * Collect all filter values from the page
         */
        ...
        performFilterAjax: function (queryId, filters) {
            ...
            if (!$grid.length) {
                console.error('LoopMosaic V9: Grid not found for:', queryId);
                return;
            }

            console.log('LoopMosaic V9: Performing AJAX', { queryId, filters });
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log('LoopMosaic V9: AJAX Success', response);
                ...
                error: function (xhr, status, error) {
                    console.error('LoopMosaic V9: AJAX Failed', { status, error });
                    $grid.removeClass('jet-filters-loading');
                }
            });
        },
        ...
        storeGridSettings: function () {
            ...
            $('.loopmosaic-grid').each(function () {
                ...
                console.log('LoopMosaic V9: Stored grid settings for:', queryId);
            });
        },
        ...
        bindEvents: function () {
            ...
            $(document).on('jet-smart-filters/inited', function () {
                console.log('LoopMosaic V9: EVENT jet-smart-filters/inited');
                self.disableNativeLinks();
            });
    ...
    $(window).on('elementor/frontend/init', function () {
        if (window.elementorFrontend && elementorFrontend.hooks) {
            elementorFrontend.hooks.addAction('frontend/element_ready/loopmosaic-grid.default', function ($scope) {
                console.log('LoopMosaic V9: Elementor Widget Ready');
