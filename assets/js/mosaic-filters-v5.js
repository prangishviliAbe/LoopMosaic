/**
 * LoopMosaic - JetSmartFilters Integration V5
 * 
 * @package LoopMosaic
...
(function ($) {
    ...
    const LoopMosaicFilters = {
         ...
         */
        init: function () {
            console.log('LoopMosaic V5: Filters Init Started');
            ...
            console.log('LoopMosaic V5: Filters Init Complete');
        },
        ...
        initJetSmartFilters: function () {
            ...
            // NOTE: The diagnostics in V4 revealed the variable is 'JetSmartFilterSettings' (Singular)
            // But we will check for both just in case.
            const getSettings = function () {
                return window.JetSmartFilterSettings || window.JetSmartFiltersSettings;
            };

            const settingsObject = getSettings();
            console.log('LoopMosaic V5: initJetSmartFilters checking settings...', {
                foundObject: !!settingsObject,
                variableName: window.JetSmartFilterSettings ? 'JetSmartFilterSettings' : (window.JetSmartFiltersSettings ? 'JetSmartFiltersSettings' : 'NONE')
            });
            ...
            const registerProvider = function () {
                ...
                if (settings && settings.providers) {
                    console.log('LoopMosaic V5: JSF Settings Found. Registering Provider.');
                    ...
                    settings.providers['loop-mosaic'] = {
                        ...
                        apply: function (queryId, filters, pagination) {
                            console.log('LoopMosaic V5: Custom Apply Called for ' + queryId);
                    ...
                    $('.loopmosaic-grid[data-provider="loop-mosaic"]').each(function () {
                        ...
                        $grid.addClass('jet-filter-provider'); // Ensure class exists
                    });

                    // Trigger re-init if JetSmartFilters is already initialized
                    if (window.JetSmartFilters && window.JetSmartFilters.initializeFilters) {
                        // Some versions require manual re-init call if added late?
                        // Usually not needed if we hook into events, but good to know.
                    }
            ...
            // Try immediately
            if (!registerProvider()) {
                console.log('LoopMosaic V5: JSF Settings NOT found. Starting Polling...');

                const interval = setInterval(function () {
                    attempts++;
                    // console.log('LoopMosaic V5: Polling for JSF Settings... Attempt ' + attempts);

                    if (registerProvider()) {
                        clearInterval(interval);
                        console.log('LoopMosaic V5: Provider Registered via Polling.');
                    } else if (attempts >= maxAttempts) {
                        clearInterval(interval);
                        console.error('LoopMosaic V5: JSF Settings GAVE UP after 3s. Main Dependency Missing?');
                    }
                }, 100);
            }
        },
        ...
        applyFiltersAjax: function (queryId, filters, pagination) {
            ...
            console.log('LoopMosaic V5: Apply Filter', { queryId, filters, page, ajaxUrl });
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log('LoopMosaic V5: Filter Success', response);
                    ...
                    if (response.success && response.data) {
                        ...
                        console.error('LoopMosaic V5: Filter Response Error', response);
                    }
                ...
                error: function (xhr, status, error) {
                    $grid.removeClass('jet-filters-loading');
                    console.error('LoopMosaic V5: Filter Request Failed', { status, error, response: xhr.responseText });
                }
            });
        },
    ...
    $(window).on('elementor/frontend/init', function () {
        if (window.elementorFrontend && elementorFrontend.hooks) {
            elementorFrontend.hooks.addAction('frontend/element_ready/loopmosaic-grid.default', function ($scope) {
                console.log('LoopMosaic V5: Elementor Widget Ready Hook');
