/**
 * LoopMosaic - JetSmartFilters Integration V4
 * 
 * @package LoopMosaic
...
(function ($) {
    ...
    const LoopMosaicFilters = {
         ...
         */
        init: function () {
            console.log('LoopMosaic V4: Filters Init Started');
            ...
            console.log('LoopMosaic V4: Filters Init Complete');
        },
        ...
        initJetSmartFilters: function () {
            ...
            console.log('LoopMosaic V4: initJetSmartFilters checking settings...', {
            ...
            console.error('LoopMosaic V4: DIAGNOSTIC - Found Global Variables:', JSON.stringify(jetVars));

            // DEBUG: Inspect JetSmartFilters object specifically
            if (window.JetSmartFilters) {
                console.error('LoopMosaic V4: DIAGNOSTIC - JetSmartFilters Object Keys:', JSON.stringify(Object.keys(window.JetSmartFilters)));
            } else {
                console.error('LoopMosaic V4: DIAGNOSTIC - window.JetSmartFilters is UNDEFINED');
            }

            // Also check Elementor Config just in case
            if (window.elementorFrontend && window.elementorFrontend.config) {
                console.error('LoopMosaic V4: DIAGNOSTIC - Elementor Config Keys:', JSON.stringify(Object.keys(window.elementorFrontend.config)));
            }

            const registerProvider = function () {
                if (window.JetSmartFiltersSettings && window.JetSmartFiltersSettings.providers) {
                    console.log('LoopMosaic V4: JSF Settings Found. Registering Provider.');
                    ...
                    window.JetSmartFiltersSettings.providers['loop-mosaic'] = {
                        ...
                        apply: function (queryId, filters, pagination) {
                            console.log('LoopMosaic V4: Custom Apply Called for ' + queryId);
            ...
            // Try immediately
            if (!registerProvider()) {
                console.log('LoopMosaic V4: JSF Settings NOT found. Starting Polling...');

                const interval = setInterval(function () {
                    attempts++;
                    console.log('LoopMosaic V4: Polling for JSF Settings... Attempt ' + attempts);

                    if (registerProvider()) {
                        clearInterval(interval);
                        console.log('LoopMosaic V4: Provider Registered via Polling.');
                    } else if (attempts >= maxAttempts) {
                        clearInterval(interval);
                        console.error('LoopMosaic V4: JSF Settings GAVE UP after 3s. Main Dependency Missing?');
                    }
                }, 100);
            }
        },
        ...
        applyFiltersAjax: function (queryId, filters, pagination) {
            ...
            console.log('LoopMosaic V4: Apply Filter', { queryId, filters, page, ajaxUrl });
            ...
            $.ajax({
                ...
                success: function (response) {
                    console.log('LoopMosaic V4: Filter Success', response);
                    ...
                    if (response.success && response.data) {
                        ...
                        console.error('LoopMosaic V4: Filter Response Error', response);
                    }
                ...
                error: function (xhr, status, error) {
                    $grid.removeClass('jet-filters-loading');
                    console.error('LoopMosaic V4: Filter Request Failed', { status, error, response: xhr.responseText });
                }
            });
        },
    ...
    $(window).on('elementor/frontend/init', function () {
        if (window.elementorFrontend && elementorFrontend.hooks) {
            elementorFrontend.hooks.addAction('frontend/element_ready/loopmosaic-grid.default', function ($scope) {
                console.log('LoopMosaic V4: Elementor Widget Ready Hook');
