...
class LoopMosaic_JetSmartFilters_Compat {
    ...
    public function __construct() {
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Compat Constructor Called\n", FILE_APPEND );
    ...
    public function intercept_jsf_ajax() {
        ...
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Interceptor HIT: Action={$action}, Provider={$provider}\n", FILE_APPEND );
        
        if ( self::PROVIDER_ID !== $provider ) {
            return;
        }

        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Interceptor: Caught LoopMosaic Request!\n", FILE_APPEND );
    ...
    public function supply_ajax_data( $data ) {
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Interceptor: Supplying Data...\n", FILE_APPEND );

        // NOW it is safe to load the provider class, as we are deep in JSF execution
        if ( ! class_exists( 'Jet_Smart_Filters_Provider_Base' ) ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Interceptor Error: Base Class still missing!\n", FILE_APPEND );
            return $data;
        }
        ...
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Interceptor: Data Supplied. Content Length: " . strlen($content) . "\n", FILE_APPEND );
    ...
    public function register_available_provider( $providers ) {
        ...
        try {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Available Provider Hook Fired: " . current_filter() . "\n", FILE_APPEND );
            ...
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Error in available provider reg: " . $e->getMessage() . "\n", FILE_APPEND );
        }
    ...
    public function register_provider_class( $providers_manager ) {
        try {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Register Provider Class Action Fired\n", FILE_APPEND );
            
            if ( ! class_exists( 'Jet_Smart_Filters_Provider_Base' ) ) {
                file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Base Class Missing\n", FILE_APPEND );
                return;
            }
            ...
            if ( class_exists( 'Jet_Smart_Filters_Provider_LoopMosaic' ) ) {
                ...
                if ( method_exists( $providers_manager, 'register_provider' ) ) {
                    ...
                    file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Provider Class Registered and AJAX Handler Attached\n", FILE_APPEND );
                } else {
                    file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Manager missing register_provider method\n", FILE_APPEND );
                }
            }
        } catch ( Exception $e ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Error in register_provider_class: " . $e->getMessage() . "\n", FILE_APPEND );
        } catch ( Error $e ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Fatal Error in register_provider_class: " . $e->getMessage() . "\n", FILE_APPEND );
        }
    }
    ...
    public function store_widget_settings( $widget ) {
        ...
        // file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Hook: store_widget_settings for " . $widget->get_name() . "\n", FILE_APPEND );

        if ( 'loopmosaic-grid' !== $widget->get_name() ) {
            return;
        }

        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Hook: store_widget_settings MATCHED loopmosaic-grid\n", FILE_APPEND );

        $settings = $widget->get_settings_for_display();
        
        if ( empty( $settings['enable_jsf'] ) || 'yes' !== $settings['enable_jsf'] ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Hook: JSF Disabled in settings\n", FILE_APPEND );
            return;
        }

        $query_id = ! empty( $settings['jsf_query_id'] ) ? $settings['jsf_query_id'] : 'default';
        
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Hook: Storing Settings for QueryID: {$query_id}\n", FILE_APPEND );
        ...
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Hook: Settings Stored for {$query_id}\n", FILE_APPEND );
    }
    ...
    public function handle_filter_request() {
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: handle_filter_request START\n", FILE_APPEND );
        
        // Verify nonce
        if ( ! check_ajax_referer( 'loopmosaic_jsf_nonce', 'nonce', false ) ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: Invalid Nonce\n", FILE_APPEND );
        ...
        file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: QueryID={$query_id}, Page={$page}\n", FILE_APPEND );
        ...
        if ( ! $settings && isset( $_POST['settings'] ) ) {
            ...
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: Settings loaded from POST\n", FILE_APPEND );
        } else {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: Settings loaded from Transient\n", FILE_APPEND );
        }
        
        if ( empty( $settings ) ) {
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: Settings EMPTY - ABORTING\n", FILE_APPEND );
        ...
        if ( function_exists( 'jet_smart_filters' ) ) {
            $filter_args = jet_smart_filters()->query->get_query_args();
            file_put_contents( LOOPMOSAIC_PATH . 'debug_log.txt', "Handles: JSF Args: " . print_r($filter_args, true) . "\n", FILE_APPEND );
        }
